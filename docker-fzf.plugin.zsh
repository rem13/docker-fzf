...
function docker-start() {
  container=$(docker container ls -a | awk 'NR>1' | grep -v 'Up' | fzf -q "$1" | awk '{print $NF}')
  test -n "$container" && docker run "$container"
}

function docker-restart() {
  container=$(docker ps | awk 'NR>1' | fzf -q "$1"  | awk '{print $NF}')
...
function docker-logs() {
  container=$(docker container ls -a | awk 'NR>1' | fzf -q "$1" | awk '{print $NF}')
